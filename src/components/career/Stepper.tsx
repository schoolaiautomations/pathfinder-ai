...
export const Stepper = ({ current, steps = ["Basics", "Education", "Interests", "Skills", "Preferences", "Review"], stepTitleText, percentText }: StepperProps) => {
  const progress = (current / (steps.length - 1)) * 100;
  const titleStr = stepTitleText ? stepTitleText.replace("{current}", String(current + 1)).replace("{total}", String(steps.length)) : `Step ${current + 1} of ${steps.length}`;
  const percentStr = percentText ? percentText.replace("{percent}", String(Math.round(progress))) : `${Math.round(progress)}% complete`;

  return (
    <div className="w-full">
      <div className="flex justify-between mb-3 text-xs sm:text-sm font-medium text-zinc-500">
        <span>{titleStr}</span>
        <span className="text-zinc-900 font-bold">{percentStr}</span>
      </div>
        ...
        {steps.map((label, i) => {
          ...
          return (
            <div key={label} className="flex flex-col items-center gap-2 flex-1">
              ...
              <span className={cn("text-xs font-medium text-center", active ? "text-zinc-950 font-bold" : "text-zinc-500")}>{label}</span>
            </div>
          );
        })}
      </div>
